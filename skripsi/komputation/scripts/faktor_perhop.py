...
Diadaptasi dari `09_faktor_perhop.py` (ditulis untuk `../analisis/`, di luar
clone repo ini) — hanya jalur berkas yang diubah (tak ada `quantalatent/` di
repo ini; keluaran ditaruh di `results/pendukung/` mengikuti konvensi skrip
lain di sini). Logikanya dipertahankan apa adanya karena sudah benar dan
sudah diverifikasi: `DIRECTIONS` di bawah cocok persis dengan
`backend/factor/run_factor.py::DIRECTIONS` (dicek 2026-08-27, tanpa selisih
satu karakter pun), dan `jaccard`/`mech_overlap`/`corruption_hits` di
`eval/fidelity.py` adalah fungsi teks murni — TIDAK bergantung pada format
artefak `prod/`/`try/` lama yang sudah dihapus (hanya bagian LAIN modul itu,
`analyze_run_dir` dkk., yang bergantung; tak dipakai di sini).
...
Keluaran -> results/pendukung/faktor_perhop.json
...
FACTOR = ROOT / "results" / "factor"
OUT = ROOT / "results" / "pendukung" / "faktor_perhop.json"
...
# skrip analisis tak perlu mengimpor modul yang menarik torch. Diverifikasi
# 2026-08-27: identik karakter-demi-karakter dengan sumbernya.
ARAH = {
